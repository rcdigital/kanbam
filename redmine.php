...
if ( $action == "loadProjects" ) {
	...
	$client->api('issue')->update($_REQUEST["id"], array( 
	   ...
	   'fixed_version_id' => $_REQUEST["story"] 
    ));
} else if ( $action == "updateTask" ) {
	$client->api('issue')->update($_REQUEST["id"], array( 
	   'subject' => $_REQUEST["subject"], 
	   'estimated_hours' => $_REQUEST["estimated_hours"], 
	   'assigned_to_id' => $_REQUEST["assigned_to_id"] 
    ));	
} else if ( $action == "loadSpentTimesByProjectId" ) {
	echo json_encode( $client->api('time_entry')->all(array(
	...
	$client->api('issue')->create( array( 
	   ...
	   'fixed_version_id' => $_REQUEST["story_id"], 
	   'tracker_id' => $_REQUEST["activity_id"], 
	   'estimated_hours' => $_REQUEST["hours"]
    ));
} else if ( $action == "addStory" ) {
    $client->api('version')->create( $_REQUEST["project_id"], array( 
        'name' => $_REQUEST["name"], 
